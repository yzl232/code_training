...
#那么用circular buffer
# 关键2点。 1 肯定要用hashtable记录node， value是set of【nodes...】的。  2 要circular queue.每分钟
...
class Solution():
    ...
    def foo(self, timeStamp, nodeID):
        if timeStamp not in self.curCount: self.curCount[timeStamp]=set()
        self.curCount[timeStamp].add(timeStamp, nodeID)
    ...
    def check(self):  #每分钟调用一次check函数，。 merge这一分钟的hashtable。 check出不符合的。
        d = {}
        for x in self.q: #merge这60个hashtable
            for key in x:

...
先把单词表示成int(位图），比如一个单词abc，那么和它配对可能的位表其实是111后面跟23个0，表示只有abc出现。这样每个单词其实把把出现的字母标为1，得到了一个int。
...
可以理解为dp,或者递推。
（1） 对所有的状态（共2^26个） 初值 dp[] 先给  For each set S of letters, find the longest word that consists of exactly those letters.
（2） 递推关键部分:
...
max(length of word * dp[(~x) & ((1 << 26) - 1)]) 为所求
